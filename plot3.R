...
#Create the plot
png_file_name <- paste(png_path, "plot3.png", sep = "")
png(filename = png_file_name)  #default size is 480 x 480 pixels
plot.new()
with(epc_df, plot(strptime(DateTime, date_tm_format), as.numeric(Sub_metering_1), type = "n", 
                  xlab = "", ylab = "Energy sub metering"))
with(epc_df, points(strptime(DateTime, date_tm_format), as.numeric(Sub_metering_1), type = "l", col = "black"))
with(epc_df, points(strptime(DateTime, date_tm_format), as.numeric(Sub_metering_2), type = "l", col = "red"))
with(epc_df, points(strptime(DateTime, date_tm_format), as.numeric(Sub_metering_3), type = "l", col = "blue"))
